...
(function (ActionAppCore, $) {
	...
	function updateCatDetails(thecatname, theDetails) {
		...
		try {
			...
			var tmpURL = tmpBaseURL + '/wp-json/actappdesigner/update-catalog';
			var tmpPayload = {
				catname: tmpNewDetailsInfo.name,
				title: tmpNewDetailsInfo.title,
				description: tmpNewDetailsInfo.details
			}
			ThisApp.apiCall({
				url: tmpURL,
				data: tmpPayload
	...
	function preLoad(theDetails) {
		...
		var tmpBaseURL = this.getParentPage().common.getBaseURL();
		var tmpMyControls = this.getIndex().controls;
		//ToDo: Make computed instead?
		tmpMyControls.resources.controlname = tmpBaseURL + '/wp-json/actappdesigner/get-catalog-res.json?catname=' + tmpcatname;
		tmpMyControls.setupinfo.controlname = tmpBaseURL + '/wp-json/actappdesigner/get-catalog-form.json?catname=' + tmpcatname;
	...
	function addCatalogHTML() {
		...
		ThisApp.input("Enter HTML name", "HTML Name", "Create HTML Resource", "")
			.then(function (theValue) {
				...
				ThisApp.apiCall({
					url: tmpThis.context.page.controller.parts.center.getBaseURL() + '/wp-json/actappdesigner/create-catalog-res',
					data: tmpRequest
				}).then(function (theReply) {
	...
	function addCatalogControl() {
		...
		ThisApp.input("Enter control name", "Control Name", "Create Control Resource", "")
			.then(function (theValue) {
				...
				//console.log('url', tmpThis.context.page.controller.parts.center.getBaseURL() + '/wp-json/actappdesigner/create-catalog-res');
				ThisApp.apiCall({
					url: tmpThis.context.page.controller.parts.center.getBaseURL() + '/wp-json/actappdesigner/create-catalog-res' ,
					data: tmpRequest
				}).then(function (theReply) {
